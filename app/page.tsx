...
export default function ReaderPage() {
  ...
  // Refs para controle de leitura e tempo
  const viewerRef = useRef<HTMLDivElement>(null);
  const renditionRef = useRef<any>(null);
  const startTimeRef = useRef<number>(Date.now());
  const hasUpdatedStreak = useRef(false);

  // 1. Carregar dados do livro (CORRIGIDO PARA EVITAR ERRO 400)
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        rendition.on("relocated", async (location: any) => {
          ...
          // Extrai o texto para o Modo Turbo
          try {
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
                  ...
                  {loadingAi ? (
                    ...
                    <div className="font-serif text-xl leading-loose">{aiResponse}</div>
                  )}
                </div>
              </motion.div>
            </>
