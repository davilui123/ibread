...
export default function ReaderPage() {
  ...
  const id = params?.id; // Garante a captura correta do ID
  ...
  // 1. Carregar dados do livro - CORREÇÃO CRÍTICA DO FILTRO .EQ('id', id)
  useEffect(() => {
    const fetchBook = async () => {
      ...
      const { data, error } = await supabase
        .from('books')
        .select('*')
        .eq('id', id) // AQUI: 'id' entre aspas é a coluna do banco, id sem aspas é o valor da URL
        .single();
  ...
  const calculateTimeRemaining = (currentLoc: number, totalLocs: number) => {
    ...
    const minutes = Math.ceil((pagesRemaining * estSecondsPerPage) / 60);
    ...
    async function init() {
      try {
        ...
        rendition.on("relocated", async (location: any) => {
          ...
          } catch (e) { console.warn("Erro ao extrair texto"); }
  ...
  return (
    <div className={`h-screen flex flex-col transition-colors duration-500 overflow-hidden 
      ...
      <header className={`px-6 py-4 border-b flex justify-between items-center z-30 
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setIsTurboMode(true)} className="p-2.5 rounded-full bg-stone-100 text-stone-500 hover:bg-black hover:text-white transition-all duration-300">
              <Zap size={16} />
            </button>
        ...
        <AnimatePresence>
          {isDrawerOpen && (
            <>
              ...
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className={`absolute right-0 top-0 h-full w-full max-w-md z-[60] shadow-2xl p-10 flex flex-col ${theme === 'comfort' ? 'bg-[#1A1A1A] text-stone-300' : 'bg-white text-stone-800'}`}>
                ...
                <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
                  <div className={`p-5 rounded-3xl mb-8 border ${theme === 'comfort' ? 'bg-stone-900 border-stone-800' : 'bg-stone-50 border-stone-100'}`}>
                    <p className="text-[10px] font-black uppercase text-stone-400 mb-3 tracking-tighter">Trecho Ativo</p>
                    <p className="text-lg font-serif italic leading-relaxed opacity-80">"{selectedText}"</p>
                  </div>
                  {loadingAi ? <div className="py-20 flex flex-col items-center gap-4"><Loader2 className="animate-spin text-stone-300" size={40} /></div> : <div className="font-serif text-xl leading-loose">{aiResponse}</div>}
                </div>
              </motion.div>
            </>
          )}
      ...
      <footer className={`px-10 py-6 border-t flex items-center justify-between gap-10 ${theme === 'comfort' ? 'bg-[#1A1A1A] border-stone-800 text-stone-500' : 'bg-white border-stone-100 text-stone-400'}`}>
        <div className="flex flex-col">
          <span className="text-[11px] font-black uppercase tracking-tighter text-stone-800 dark:text-stone-200">Pág. {currentPage} de {totalPages}</span>
        ...
        <div className="flex-1 h-[2px] bg-stone-100 rounded-full overflow-hidden relative">
          <motion.div className={`h-full ${theme === 'comfort' ? 'bg-stone-600' : 'bg-black'}`} initial={{ width: 0 }} animate={{ width: `${(currentPage / (totalPages || 1)) * 100}%` }} />
        </div>
